...
class VectorBaseDocument(BaseModel, Generic[T], ABC):
    ...
    def bulk_insert(cls: Type[T], documents: list["VectorBaseDocument"]) -> bool:
        try:
            ...
            logger.info(
                f"[VECTOR] Collection '{cls.get_collection_name()}' does not exist. Trying to create the collection and reinsert the documents."
            )
            ...
            try:
                ...
                logger.error(f"[VECTOR] Failed to insert documents in '{cls.get_collection_name()}'.")
    ...
    def bulk_find(cls: Type[T], limit: int = 10, **kwargs) -> tuple[list[T], UUID | None]:
        try:
            ...
            logger.error(f"[VECTOR] Failed to search documents in '{cls.get_collection_name()}'.")
    ...
    def get_or_create_collection(cls: Type[T]) -> CollectionInfo:
        ...
        try:
            ...
            if collection_created is False:
                raise RuntimeError(f"[VECTOR] Couldn't create collection {collection_name}") from None
