...
from domain.chunks import Chunk, SiteChunk
...
class SiteEmbeddingHandler(EmbeddingDataHandler):
  def map_model(self, data_model: SiteChunk, embedding: list[float]) -> EmbeddedSiteChunk:
    return EmbeddedSiteChunk(
      id=data_model.id,
      content=data_model.content,
      embedding=embedding,
